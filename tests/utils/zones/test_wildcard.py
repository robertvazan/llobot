...
def test_wildcard_zoning():
    ...
    assert zoning.resolve(PurePosixPath("app1")) == Path("/tmp/data/app1/logs")
    assert zoning[PurePosixPath("app2")] == Path("/tmp/data/app2/logs")
...
def test_wildcard_zoning_with_path():
    zoning = WildcardZoning("/tmp/data/*/logs")
    assert zoning.resolve(PurePosixPath("app1/v1")) == Path("/tmp/data/app1/v1/logs")
    assert zoning[PurePosixPath("app2/v2")] == Path("/tmp/data/app2/v2/logs")
