...
def test_prefix_zoning():
    ...
    assert zoning.resolve(PurePosixPath("zone1")) == Path("/tmp/data/zone1")
    assert zoning[PurePosixPath("zone2")] == Path("/tmp/data/zone2")

def test_prefix_zoning_with_path():
    zoning = PrefixZoning("/tmp/data")
    assert zoning.resolve(PurePosixPath("zone1/sub")) == Path("/tmp/data/zone1/sub")
    assert zoning[PurePosixPath("zone2/sub")] == Path("/tmp/data/zone2/sub")
