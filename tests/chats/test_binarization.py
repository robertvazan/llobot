...
def test_binarize_chat_last_prompt():
    ...
    assert binarize_chat(chat, last=ChatIntent.PROMPT).messages == (ChatMessage(ChatIntent.PROMPT, "p1"),)

    chat = ChatThread([ChatMessage(ChatIntent.RESPONSE, "r1")])
...
def test_binarize_chat_last_response():
    ...
    chat = ChatThread([ChatMessage(ChatIntent.RESPONSE, "r1")])
    assert binarize_chat(chat, last=ChatIntent.RESPONSE).messages == (ChatMessage(ChatIntent.RESPONSE, "r1"),)

    chat = ChatThread([ChatMessage(ChatIntent.PROMPT, "p1")])
    binarized = binarize_chat(chat, last=ChatIntent.RESPONSE)
    assert binarized.messages == (
        ChatMessage(ChatIntent.PROMPT, "p1"),
        ChatMessage(ChatIntent.RESPONSE, "Okay"),
    )
