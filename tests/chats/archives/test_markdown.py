...
def test_add_read(tmp_path: Path):
    ...
    archive.add(Path("zone1/sub"), now, chat)

    read_chat = archive.read(Path("zone1/sub"), now)
    assert read_chat is not None
    assert read_chat == chat

    assert archive.read(Path("zone1/sub"), now + timedelta(seconds=1)) is None
    assert archive.read(Path("zone2"), now) is None
...
def test_contains(tmp_path: Path):
    ...
    now = current_time()
    chat = create_test_chat("Hello")
    archive.add(Path("zone1"), now, chat)

    assert archive.contains(Path("zone1"), now)
    assert not archive.contains(Path("zone1"), now + timedelta(seconds=1))
    assert not archive.contains(Path("zone2"), now)
...
def test_remove(tmp_path: Path):
    ...
    archive.remove(Path("zone1"), now)
    assert not archive.contains(Path("zone1"), now)
...
def test_scatter(tmp_path: Path):
    ...
    archive.scatter([Path("zone1"), Path("zone2/sub")], now, chat)

    assert archive.read(Path("zone1"), now) == chat
    assert archive.read(Path("zone2/sub"), now) == chat
...
def test_recent(tmp_path: Path):
    ...
    archive.add(Path("zone"), time1, chat1)

    chat2 = create_test_chat("2")
    time2 = now - timedelta(seconds=1)
    archive.add(Path("zone"), time2, chat2)

    chat3 = create_test_chat("3")
    time3 = now
    archive.add(Path("zone"), time3, chat3)

    recent_chats = list(archive.recent(Path("zone")))
...
def test_recent_with_cutoff(tmp_path: Path):
    ...
    recent_chats = list(archive.recent(Path("zone"), cutoff=time1))
    assert len(recent_chats) == 1
    assert recent_chats[0] == (time1, chat1)

    recent_chats_2 = list(archive.recent(Path("zone"), cutoff=time2))
...
def test_last(tmp_path: Path):
    ...
    assert archive.last(Path("zone")) == (None, None)
    ...
    assert archive.last(Path("zone")) == (time2, chat2)
...
def test_last_with_cutoff(tmp_path: Path):
    ...
    assert archive.last(Path("zone"), cutoff=time1) == (time1, chat1)
    assert archive.last(Path("zone"), cutoff=now - timedelta(seconds=2)) == (None, None)
