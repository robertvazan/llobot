...
def test_add_read(tmp_path: Path):
    ...
    history.add(PurePosixPath("zone1/sub"), now, chat)

    read_chat = history.read(PurePosixPath("zone1/sub"), now)
    assert read_chat is not None
    assert read_chat == chat

    assert history.read(PurePosixPath("zone1/sub"), now + timedelta(seconds=1)) is None
    assert history.read(PurePosixPath("zone2"), now) is None
...
def test_contains(tmp_path: Path):
    ...
    now = current_time()
    chat = create_test_chat("Hello")
    history.add(PurePosixPath("zone1"), now, chat)

    assert history.contains(PurePosixPath("zone1"), now)
    assert not history.contains(PurePosixPath("zone1"), now + timedelta(seconds=1))
    assert not history.contains(PurePosixPath("zone2"), now)
...
def test_remove(tmp_path: Path):
    ...
    history.remove(PurePosixPath("zone1"), now)
    assert not history.contains(PurePosixPath("zone1"), now)
...
def test_scatter(tmp_path: Path):
    ...
    history.scatter([PurePosixPath("zone1"), PurePosixPath("zone2/sub")], now, chat)

    assert history.read(PurePosixPath("zone1"), now) == chat
    assert history.read(PurePosixPath("zone2/sub"), now) == chat
...
def test_recent(tmp_path: Path):
    ...
    history.add(PurePosixPath("zone"), time1, chat1)

    chat2 = create_test_chat("2")
    time2 = now - timedelta(seconds=1)
    history.add(PurePosixPath("zone"), time2, chat2)

    chat3 = create_test_chat("3")
    time3 = now
    history.add(PurePosixPath("zone"), time3, chat3)

    recent_chats = list(history.recent(PurePosixPath("zone")))
...
def test_recent_with_cutoff(tmp_path: Path):
    ...
    recent_chats = list(history.recent(PurePosixPath("zone"), cutoff=time1))
    assert len(recent_chats) == 1
    assert recent_chats[0] == (time1, chat1)

    recent_chats_2 = list(history.recent(PurePosixPath("zone"), cutoff=time2))
...
def test_last(tmp_path: Path):
    ...
    assert history.last(PurePosixPath("zone")) == (None, None)
    ...
    assert history.last(PurePosixPath("zone")) == (time2, chat2)
...
def test_last_with_cutoff(tmp_path: Path):
    ...
    assert history.last(PurePosixPath("zone"), cutoff=time1) == (time1, chat1)
    assert history.last(PurePosixPath("zone"), cutoff=now - timedelta(seconds=2)) == (None, None)
