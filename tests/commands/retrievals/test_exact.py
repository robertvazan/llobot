...
def test_multiple_matches(tmp_path):
    # exact retrieval only matches one path, but we can setup
    # two files and request one.
    knowledge = {
        PurePosixPath('a/b.txt'): 'content',
        PurePosixPath('x/b.txt'): 'another content',
    }
    env = create_env(tmp_path, knowledge)
    assert handle_exact_retrieval_command('project/a/b.txt', env)
...
def test_exact_match(tmp_path):
    env = create_env(tmp_path, KNOWLEDGE)
    assert handle_exact_retrieval_command('project/d.txt', env)
...
def test_absolute_path_fails(tmp_path):
    env = create_env(tmp_path, KNOWLEDGE)
    assert not handle_exact_retrieval_command('/project/a/b.txt', env)
    assert not env[RetrievalsEnv].get()
