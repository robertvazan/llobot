...
def create_tree_format(function: Callable[[KnowledgeTree], str]) -> TreeFormat:
    """
    Creates a tree format from a function.

    Args:
        function: Function that takes a KnowledgeTree and returns a Markdown string.

    Returns:
        A TreeFormat that uses the provided function.
    """
    class LambdaTreeFormat(TreeFormat):
        def render(self, tree: KnowledgeTree) -> str:
            return function(tree)
    return LambdaTreeFormat()

@lru_cache
def flat_tree_format(title: str) -> TreeFormat:
    """
    Creates a tree format that lists all file paths in a flat list.
    ...
    Returns:
        A TreeFormat that renders paths as a flat list.
    """
    ...
    return create_tree_format(render_flat)

@lru_cache
def grouped_tree_format(title: str) -> TreeFormat:
    """
    Creates a tree format that groups files by directory.
    ...
    Returns:
        A TreeFormat that renders paths grouped by directory.
    """
    ...
    return create_tree_format(render_grouped)

@lru_cache
def standard_tree_format(title: str = 'Files') -> TreeFormat:
    """
    Creates the standard tree format to be used by default.

    The standard format groups files by directory with a default title of 'Files'.

    Args:
        title: Title to use for the details/summary section.

    Returns:
        The standard TreeFormat (currently grouped formatting).
    """
    return grouped_tree_format(title)
