...
class Agent(Role):
    ...
    def chat(self, prompt: ChatThread) -> ChatStream:
        ...
        5. It checks for any status messages generated by commands. If found, it
           streams them. Otherwise, it generates a response from the model.
        6. It saves the session state.
        ...
        # If any status messages were added, stream them and stop.
        if any(m.intent == ChatIntent.STATUS for m in new_messages):
