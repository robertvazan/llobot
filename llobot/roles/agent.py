...
class Agent(Role):
    ...
    def chat(self, prompt: ChatThread) -> ChatStream:
        ...
        if not env[PromptEnv].swallowed:
            pre_generation_len = len(env[ContextEnv].build())
            ...
            if self._autonomy.autorun and self._tools:
                # Get the newly generated response messages
                full_context = env[ContextEnv].build()
                new_messages = full_context[pre_generation_len:]

                # Find the last response message in the newly generated content
                response = next((m for m in reversed(new_messages) if m.intent == ChatIntent.RESPONSE), None)

                if response:
                    # Capture context length before execution to identify new status messages
                    pre_execution_len = len(env[ContextEnv].build())

                    # Execute tool calls. We ignore the return value (count) because
                    # autorun does not error out if there are no tool calls.
                    execute_tool_calls(env, response.content)

                    # Stream any status messages generated by tool execution
                    new_messages = env[ContextEnv].build()[pre_execution_len:]
                    for message in new_messages:
                        if message.intent == ChatIntent.STATUS:
                            yield from message.stream()
