...
def echo(context_size: int = 16*1024, *, token_length: float = 0.0, aliases: Iterable[str] = []) -> Model:
    ...
    class EchoModel(Model):
        _context_size: int
        _token_length: float
        def __init__(self, context_size: int, token_length: float):
        ...
        @property
        def aliases(self) -> Iterable[str]:
            yield f'echo:{self._context_size // 1024}k'
            yield from aliases
        @property
        def options(self) -> dict:
            return {
        ...
        def validate_options(self, options: dict):
            allowed = {'context_size', 'token_length'}
    ...
    return EchoModel(context_size, token_length)
