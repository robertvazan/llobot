...
from llobot.models.ollama.encoding import encode_ollama_request, parse_ollama_stream
...
class _OllamaModel(Model):
    ...
    def generate(self, prompt: ChatBranch) -> ModelStream:
        ...
        def _stream() -> ModelStream:
            ...
            request = encode_ollama_request(self._name, options, sanitized_prompt)
            with requests.post(self._endpoint + '/chat', stream=True, json=request) as http_response:
                http_response.raise_for_status()
                yield ChatIntent.RESPONSE
                yield from parse_ollama_stream(http_response.iter_lines())
        return buffer_stream(_stream())

# Default tag is :latest.
def ollama_model(name: str, num_ctx: int, **kwargs) -> Model:
